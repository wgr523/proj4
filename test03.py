...
class TestStringMethods(unittest.TestCase):
    ...
    def todo(self, url, key, value):
        try:
            r=requests.post(url,data={'key': '1'+key, 'value': value,'requestid':random.randint(0,10000000)})
            print(r.text)
        except:
            print('wrong')
    ...
    def testinsert(self):
        tt=[]
        for i in range(6):
            t=threading.Thread(target=self.todo,args=(self.list_of_address_long[i%3]+self.path_insert,str(i+1), '_'))
            t.start()
            tt.append(t)
        for t in tt:
            t.join()
        tt=[]
        for i in range(6):
            t=threading.Thread(target=self.todo,args=(self.list_of_address_long[i%3]+self.path_update,str(i+1), 'q'))
            t.start()
            tt.append(t)
        for t in tt:
            t.join()
        tt=[]
        for i in range(6):
            t=threading.Thread(target=self.todo,args=(self.list_of_address_long[i%3]+self.path_delete,str(i+1), None))
            t.start()
            tt.append(t)
        try:
